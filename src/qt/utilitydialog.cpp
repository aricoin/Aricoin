    ...
    ui(new Ui::AboutDialog)
{
    ...
    ui->copyrightLabel->setText(tr("Copyright") + QString(" &copy; 2009-%1 ").arg(COPYRIGHT_YEAR) + tr("The Aricoin Core developers") + QString("<br>") + tr("Copyright") + QString(" &copy; 2013-%1 ").arg(COPYRIGHT_YEAR) + tr("The Aricoin developers"));
}
...
void PaperWalletDialog::on_getNewAddress_clicked()
{
    ...
    CAricoinAddress pubkeyhash;
    pubkeyhash.Set(pubkey.GetID());

    // Create String versions of each
    string myPrivKey = CAricoinSecret(privKey).ToString();
...
void PaperWalletDialog::on_printButton_clicked()
{
   ...
   // Stolen from sendcoinsdialog.cpp
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(AricoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    foreach(AricoinUnits::Unit u, AricoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(AricoinUnits::formatWithUnit(u, totalAmount));
    }

    questionString.append(tr("Total Amount %1 (= %2)")
        .arg(AricoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    header = tr("Aricoin Core") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  aricoin-qt [" + tr("command-line options") + "]                     " + "\n";

    coreOptions = QString::fromStdString(HelpMessage(HMM_ARICOIN_QT));
    ...
    ui->helpMessageLabel->setFont(GUIUtil::aricoinAddressFont());
...
/** "Shutdown" window */
void ShutdownWindow::showShutdownWindow(AricoinGUI *window)
{
    if (!window)
        return;
    ...
    layout->addWidget(new QLabel(
        tr("Aricoin Core is shutting down...") + "<br /><br />" +
