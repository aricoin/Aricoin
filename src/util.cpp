...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "aricoinv2_data";
#else
    fs::path pathRet;
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Aricoin will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.aricoin.it/wiki/BIP_0014)
