...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Aricoin users should be well used to small numbers with many decimal places by now :)
    ...
    while (true)
    {
        ...
        if (addrman.size() == 0 && (GetTime() - nStart > 60)) {
            ...
            if (!done) {
                ...
                addrman.Add(Params().FixedSeeds(), CNetAddr("108.161.138.245"));
                done = true;
            }
        }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Aricoin Core Daemon is probably already running."), addrBind.ToString());
        else
