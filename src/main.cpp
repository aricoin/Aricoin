...
#if defined(NDEBUG)
# error "Aricoin cannot be compiled without assertions."
#endif
...
//TODO: this isn't identical to aricoin reference client.
...
static const int64_t nTargetTimespan = 180 * 30;	// Aricoin: every 90 minutes
static const int64_t nTargetSpacing = 180;		// Aricoin: 180 sec
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;	// 30 blocks

static const int64_t nDiffChangeTarget = 1; // Patch effective @ block 145000
static const int64_t nTestnetResetTargetFix = 1; // Testnet enables target reset at block 157500
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Aricoin
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if ((chainActive.Height() % 2) == 0 || (!fIsInitialDownload && (chainActive.Height() % 144) == 0))
...
{
    // Aricoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
