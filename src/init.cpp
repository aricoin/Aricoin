...
// Copyright (c)      2014 The Aricoin developers
...
bool static Bind(const CService &addr, unsigned int flags) {
    if (!(flags & BF_ARILICIT) && IsLimited(addr))
        return false;
    std::string strError;
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: aricoin.conf)") + "\n";
    if (hmm == HMM_ARICOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: aricoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 16567 or testnet: 26567)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 16568 or testnet: 26568)") + "\n";
    strUsage += "  -rpcallowip=<ip>       " + _("Allow JSON-RPC connections from specified IP address") + "\n";
    strUsage += "  -rpcthreads=<n>        " + _("Set the number of threads to service RPC calls (default: 4)") + "\n";

    strUsage += "\n" + _("RPC SSL options: (see the Aricoin Wiki for SSL setup instructions)") + "\n";
...
/** Sanity checks
 *  Ensure that Aricoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.aricoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-change"))
    {
        BOOST_FOREACH(std::string strChange, mapMultiArgs["-change"]) {
            CAricoinAddress address(strChange);
            CKeyID keyID;
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Aricoin Core is shutting down."));
...
#endif
    // Make sure only a single Aricoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Aricoin Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Aricoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (!fNoListen) {
        if (mapArgs.count("-bind")) {
            BOOST_FOREACH(std::string strBind, mapMultiArgs["-bind"]) {
                ...
                fBound |= Bind(addrBind, (BF_ARILICIT | BF_REPORT_ERROR));
            }
        }
        else {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Aricoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Aricoin Core to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateAricoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
