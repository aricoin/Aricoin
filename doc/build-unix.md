UNIX BUILD NOTES
====================
Some notes on how to build Aricoin in Unix. 
...
This will build aricoin-qt as well if the dependencies are met.
...
memory available when compiling Aricoin Core. With 512MB of memory or less
...
If you want to build Aricoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a aricoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip aricoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the aricoin directory
BDB_PREFIX="${ARICOIN_ROOT}/db5"
mkdir -p $BDB_PREFIX
...
# Configure Aricoin Core to use our own-built instance of BDB
cd $ARICOIN_ROOT
...
cryptography functions, and therefore cannot be used for Aricoin (or other
cryptocurrencies based on the Aricoin design). Further details are available
on the Aricoin Wiki: https://en.aricoin.it/wiki/OpenSSL_and_EC_Libraries
...
Security
--------
To help make your aricoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, aricoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, aricoin may be compiled in
