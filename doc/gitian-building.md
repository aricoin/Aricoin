...
*Setup instructions for a gitian build of Aricoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aricoin
...
- [Building Aricoin](#building-aricoin)
...
Clone the git repositories for aricoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aricoin/aricoin
```
...
In [doc/release-process.md](release-process.md) in the aricoin repository under 'Fetch and build inputs'.
...
To build Aricoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the aricoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aricoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/aricoin/aricoin
...
For example:
```bash
URL=https://github.com/langerhans/aricoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aricoin=${COMMIT} --url aricoin=${URL} ../aricoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aricoin=${COMMIT} --url aricoin=${URL} ../aricoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/aricoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aricoin-build.assert
```
...
[aricoin/gitian.sigs](https://github.com/aricoin/gitian.sigs/) repository, or
