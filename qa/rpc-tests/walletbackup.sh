#!/usr/bin/env bash
# Copyright (c) 2014 The Aricoin Core developers
# Copyright (c) 2014 The Aricoin Core developers
...
# 1 block, 500000 XDG each == 500000 ARI
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} ari"
...
function StartThree {
  $ARICOIND $ARICOINDARGS $B1ARGS &
  B1PID=$!
  $ARICOIND $ARICOINDARGS $B2ARGS &
  B2PID=$!
  $ARICOIND $ARICOINDARGS $B3ARGS &
  B3PID=$!
}
